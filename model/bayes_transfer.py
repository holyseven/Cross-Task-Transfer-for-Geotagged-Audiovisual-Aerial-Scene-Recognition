...
def net_test(net,data_loader, scene_to_event, iteration, cal_map=False):
    ...
    with paddle.no_grad():
        for i, data in enumerate(data_loader, 0):
            ...
            img, aud, label = img.type(paddle.FloatTensor).cuda(), aud.type(paddle.FloatTensor).cuda(), label.type(paddle.LongTensor).cuda() # gpu
            
            #scene_to_event_ = np.tile(scene_to_event, (sample_num,1,1))
            #scene_to_event_cuda = paddle.to_tensor(scene_to_event_).cuda()
            
            output = net(img, aud)
            scene_prob   = paddle.nn.functional.softmax(output, axis=1)
            event_output  = scene_prob.matmul(scene_to_event)
...
class cosine_loss(paddle.nn.Layer):
    def __init__(self):
        super(cosine_loss, self).__init__()
        self.cosine = paddle.nn.CosineSimilarity(axis=1)

    def forward(self, pred, true):
        return paddle.mean(1. - self.cosine(pred, true))
...
def main():
    ...
    fusion_net_cuda = paddle.DataParallel(fusion_net, device_ids=gpu_ids)

    loss_func_CE  = paddle.nn.CrossEntropyLoss()
    loss_func_BCE = paddle.nn.BCELoss(reduce=True)
    ...
    scene_to_event = paddle.to_tensor(scene_to_event).cuda()
    #scene_to_event = paddle.unsqueeze(x, 0)
    ...
    for e in range(args.epoch):
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            img, aud, scene_label, event_label, event_corr = paddle.cast(img, dtype='float32').cuda(), paddle.cast(aud, dtype='float32').cuda(), \
                paddle.cast(scene_label, dtype='int64').cuda(), paddle.cast(event_label, dtype='float32').cuda(), paddle.cast(event_corr, dtype='float32').cuda()
            ...
            if args.using_event_knowledge:
                scene_prob    = paddle.nn.functional.softmax(scene_output, axis=1)
                event_output  = scene_prob.matmul(scene_to_event)           
